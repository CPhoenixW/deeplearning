...
def test_rq3_matrix_has_explicit_mechanism_endpoints(tmp_path: Path) -> None:
    ...
    assert len(specs) == 51
    assert sum(item[1] == "none" and item[2] == 0.0 for item in specs) == 3
    ...
    assert by_mechanism["p1_only"]["fed_config_overrides"]["phase1_score_mode"] == "recon"
    assert by_mechanism["p1_only"]["fed_config_overrides"]["phase2_score_mode"] == "recon"
    assert by_mechanism["p2_only"]["fed_config_overrides"]["phase1_score_mode"] == "recon"
    assert by_mechanism["p2_only"]["fed_config_overrides"]["phase2_score_mode"] == "combined"
    assert by_mechanism["full"]["fed_config_overrides"]["phase1_score_mode"] == "recon"
    assert by_mechanism["full"]["fed_config_overrides"]["phase2_score_mode"] == "combined"
    assert by_mechanism["full"]["fed_config_overrides"]["svdd_lambda"] == 0.5
...
def test_rq3_old_svdd_phase2_results_are_not_reused(tmp_path: Path) -> None:
    kwargs = dict(
        task="fashion_mnist",
        mechanisms=("p2_only", "full"),
        attacks=("gn",),
        ratios=(0.1,),
        seeds=(42,),
        rounds=20,
    ...
    first = build_jobs(tmp_path / "rq3", **kwargs)
    assert len(first) == 4
    for _mechanism, attack, ratio, _seed, config_path, output_dir in first:
        output_dir.mkdir(parents=True, exist_ok=True)
        payload = json.loads(config_path.read_text(encoding="utf-8"))
        effective = {
            **payload["fed_config_overrides"],
            "phase2_score_mode": "svdd",
            "num_clients": 100,
            "num_benign": 100 if attack == "none" else 100 - int(round(100 * ratio)),
        }
        result = {
            "meta": {
                "task": "fashion_mnist",
        ...
        (output_dir / f"fashion_mnist__{attack}__svdd.json").write_text(
