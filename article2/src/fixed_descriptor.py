...
def _allocate_layer_dims(total: int, sizes: Sequence[int]) -> List[int]:
    if total < len(sizes):
        raise ValueError(
            f"Layer projection needs at least one coordinate per layer: {total} < {len(sizes)}."
        )
...
class FixedHierarchicalMultiViewDescriptor:
    ...
    def __init__(
        ...
        *,
        ...
        global_dim = self.output_dim // 2
        statistics_dim = self.output_dim // 8
        layer_dim = self.output_dim - global_dim - statistics_dim
    ...
    def describe(
        ...
        flat_delta = torch.cat(flat_parts).to(self.projection_device, non_blocking=True)
        stats_vector = torch.cat(statistics).to(self.projection_device, non_blocking=True)
        global_bucket, global_sign, layer_bucket, layer_sign = self._projection_tensors

        global_view = torch.zeros(
            self.layout.global_dim,
            device=self.projection_device,
            dtype=torch.float32,
        )
        global_view.scatter_add_(0, global_bucket, flat_delta * global_sign)

        layer_view = torch.zeros(
            self.layout.layer_dim,
            device=self.projection_device,
            dtype=torch.float32,
        )
        layer_view.scatter_add_(0, layer_bucket, flat_delta * layer_sign)
        statistics_view = self._statistics_matrix @ stats_vector
