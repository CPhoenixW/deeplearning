...
<main><h1>Federated experiment monitor</h1><div class=sub id=updated>Loading…</div><div class=summary id=summary></div><table><thead><tr><th>Attack</th><th>Defense</th><th>Seed</th><th>Status</th><th>Round</th><th>TACC</th><th>ASR</th><th>RR</th></tr></thead><tbody id=rows></tbody></table></main>
<script>const f=v=>v==null?'—':(100*v).toFixed(2)+'%';async function load(){let d=await fetch('/api/status',{cache:'no-store'}).then(r=>r.json());document.querySelector('#updated').textContent=`Updated ${new Date(d.updated_at).toLocaleTimeString()} · ${d.root}`;document.querySelector('#summary').innerHTML=Object.entries(d.counts).map(([k,v])=>`<div class=card><b>${v}</b>${k}</div>`).join('');document.querySelector('#rows').innerHTML=d.jobs.map(j=>`<tr><td>${j.attack}</td><td>${j.defense}</td><td>${j.seed}</td><td class=${j.status}>${j.status}</td><td>${j.round}/${j.total_rounds}</td><td>${f(j.accuracy)}</td><td>${f(j.asr)}</td><td>${f(j.rr)}</td></tr>`).join('')}load();setInterval(load,5000)</script>"""
...
def status(root: Path) -> dict:
    jobs = []
    for config_path in sorted(root.glob("_configs/*/seed_*/*.json")):
        try:
            ...
            continue
        directory = root / defense / f"seed_{seed}"
        ...
        record = {"attack": attack, "defense": defense, "seed": seed,
                  ...
                  "asr": None, "rr": None, "status": "pending"}
        try:
        ...
        if _failed(directory / f"{attack}.log"):
