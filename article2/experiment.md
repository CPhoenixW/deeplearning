...
2. **RQ2：单一攻击检测。** 面对不同类型的单一攻击时，本文方法能否准确区分恶意客户端与良性客户端？
3. **RQ3：混合攻击检测。** 当不同恶意客户端在同一轮实施不同攻击时，本文方法能否同时识别各攻击族，而非只对其中一种攻击有效？
4. **RQ4：两阶段机制。** 第一阶段重建误差筛选和第二阶段潜在空间检测与可信聚合分别发挥什么作用，二者是否具有互补性？
5. **RQ5：恶意比例。** 当恶意客户端比例在良性多数假设内变化时，本文方法的效用和检测能力如何变化？
6. **RQ6：客户端规模。** 客户端数量从 50 增加到 200 时，本文方法的检测效果、模型效用和执行开销是否稳定？
7. **RQ7：数据异质性。** 不同程度的 non-IID 是否会使正常 client drift 被误判为恶意行为？
8. **RQ8：攻击强度。** 从弱攻击到强攻击，检测效果的变化来自防御能力还是攻击本身尚未成功？
9. **RQ9：动态阈值。** 第二阶段的动态收紧阈值是否优于固定阈值？
10. **RQ10：特征压缩。** 固定投影维度如何影响检测信息保留与计算代价之间的权衡？
