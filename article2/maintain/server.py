...
# 温度 T：从极其平缓到极其陡峭（用于软权重）
T_START = 5.0
T_END = 0.5

# 边界系数 k：从极度包容到极度严格（用于硬截断阈值 tau）
K_START = 3.0
K_END = 1.0

# SVDD 阶段梯度裁剪阈值（Encoder），防止梯度爆炸
SVDD_GRAD_CLIP_NORM = 5.0
...
class FederatedServer:
    ...
    def aggregate(
        ...
        if round_num <= PHASE1_END_ROUND:
            ...
            # Phase 1 使用完整的 AE 学习率
            for g in self.optimizer_ae.param_groups:
                g["lr"] = self.lr_ae
            self.ae.train()
            ...
            # SVDD 训练阶段：不再使用距离作为权重，只在“阈值内”的客户端上做均匀约束
            svdd_mask = M
            svdd_norm = svdd_mask.sum() + 1e-12
            svdd_loss = (svdd_mask * d_enc).sum() / svdd_norm
            svdd_loss.backward()
            clip_grad_norm_(self.ae.encoder.parameters(), max_norm=SVDD_GRAD_CLIP_NORM)
            self.optimizer_ae.step()
