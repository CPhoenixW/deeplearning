...
在目标服务器上使用正式 K=100 和上述批量参数完成一轮容量验证，四个任务均无 OOM：MNIST 约 11 秒/轮、Fashion-MNIST 约 12 秒/轮、CIFAR-10 约 17 秒/轮、AG News 约 10 秒/轮。AG News 每个独立进程还需约 1 分钟构建词表，这段预处理不包含在结果 JSON 的训练时间内。按当前实测，A1 预计约 6--8 小时，A2 预计约 13--16 小时，总墙钟时间约 20 小时；实际值会随共享服务器负载变化。
